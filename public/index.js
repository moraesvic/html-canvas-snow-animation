...
function randfloatGaussian(a, b) {
  var sum = 0;
  for (var i = 0; i < GAUSSIAN_SAMPLE; i++) sum += Math.random();
...
function RGBAToString(rgb, a) {
  var retString = "rgb(" + rgb.r + "," + rgb.g + "," + rgb.b + "," + a + ")";
  return retString;
}
...
class Snowflake {
  constructor() {
    ...
    this.size = Math.floor(
      ((this.radius - SF.minRadius) / (SF.maxRadius - SF.minRadius)) *
        SNOW_PNG.length
    );
  ...
  draw(ctx) {
    if (
      this.x > 1.1 * canvasWidth ||
      this.x < -0.1 * canvasWidth ||
      this.y > 1.1 * canvasHeight ||
  ...
  move() {
    ...
    this.y += this.yVel;
    if (this.radius < 1.5) this.y -= 0.8 * randfloat(SF.minYVel, SF.maxYVel);

    this.t++;
    if (this.t > 1200) this.transparency -= 0.005;
  }
}
  ...
  for (var loopNumber = 0; ; loopNumber++) {
    ...
    if (loopNumber % LOOP_CLEAR === 0) {
      ...
      for (var i = 0; i < flakes.length; i++)
        if (flakes[i].transparency <= 0.0) flakes.splice(i, 1);
    }
...
function playAudio() {
  ...
  var elem;
  if (!isChrome) elem = document.getElementById("iframeAudio");
  else elem = document.getElementById("playAudio");
